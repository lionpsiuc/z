...
// inline void set_elapsed_time(cudaEvent_t start, cudaEvent_t end, float *const
// timings, int* index)
// {
//     if (timings != NULL)
//     {
 ...
 */
inline __host__ __device__ int allocation(const int size, const int bins,
                                          const int index) {
 ...
 */
inline __host__ __device__ int offset(const int size, const int bins,
                                      const int index) {
  const int quot = size / bins;
  const int rem  = size % bins;
