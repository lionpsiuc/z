 ...
 * written.
 */
static __global__ void average_rows_kernel(const int height, const int width,
                                           const int            stride,
                                           const FP_TYPE* const matrix_gpu,
                                           FP_TYPE* const       averages_gpu) {
  /* Declare shared memory */

  extern __shared__ FP_TYPE
      thread_averages[]; /* Averages computed by each thread */
  __shared__ FP_TYPE
      block_average; /* The sum thread_averages across the block */

  /* Grid-level constants */

  // These would be the x, y dimensions if we used a 2D grid.
  const int blocks_per_row = div_up(
      width, MAX_ENTRIES_PER_THREAD *
                 blockDim.x); /* The number of blocks working on a given row */
  const int rows_per_grid =
      gridDim.x /
      blocks_per_row; /* The number of rows the grid can work on at once */
  ...
  const int block_steps = div_up(
      height - start_row, rows_per_grid); /* The number of rows to average */

  /* Thread-level constants */

  const int thread_row_id =
      block_row_index * blockDim.x +
      threadIdx.x; /* The index among threads on the same row */
  const int thread_stride =
      blocks_per_row *
      blockDim.x; /* The gap between subsequent entries to average */
  const int thread_steps =
      div_up(width - thread_row_id,
             thread_stride); /* The number of entries to average per row */
  ...
  for (int i = 0; i < block_steps; i++) {
    ...
    const int block_offset =
        stride * row +
        blockDim.x *
            block_row_index; /* The first element for thread 0 to work on */
    const int thread_offset =
        block_offset +
        threadIdx.x; /* The first element for the thread to work on */
    ...
    for (int i = 0; i < thread_steps; i++) {
      thread_averages[threadIdx.x] +=
          matrix_gpu[i * thread_stride + thread_offset];
    }
...
int average_rows_gpu(const int height, const int width, const int stride,
  ...
  if (stride < width) {
    fprintf(
        stderr,
        "Bad call to average_rows_gpu() (width = %d, stride = %d). Exiting.\n",
        width, stride);
    exit(EXIT_FAILURE);
  }
  ...
  const int blocks_per_row = div_up(
      ...
      MAX_ENTRIES_PER_THREAD *
          block_size); /* The number of blocks that will work on a given row */
  const int block_allocation_size =
      block_size * sizeof(FP_TYPE); /* The size of each block's dynamically
                                       allocated shared memory */

  // We choose this to be a multiple of blocks_per_row
  const int max_grid_size =
      (current_max_grid_dim_x() / blocks_per_row) *
      blocks_per_row; /* The maximum x dimension of a grid */
  const int grid_size =
      MIN(max_grid_size, blocks_per_row * height); /* The x dimension to use */

  // printf("Row decomp:\t%d = %d x %d\n", width, blocks_per_row,
  // MAX_ENTRIES_PER_THREAD * block_size); printf("Grid dims:\t%d x %d\n",
  ...
  if (cudaMalloc((void**) (&matrix_gpu), height * stride * sizeof(FP_TYPE)) !=
      cudaSuccess) {
  ...
  if (cudaMalloc((void**) (&averages_gpu), height * sizeof(FP_TYPE)) !=
      cudaSuccess) {
    fprintf(stderr, "Failed to allocate memory on device. Exiting.\n");
    exit(EXIT_FAILURE);
  }
  ...
  TIME_START();
  cudaMemcpy(matrix_gpu, matrix, height * stride * sizeof *matrix_gpu,
  ...
  TIME_START();
  average_rows_kernel<<<grid_size, block_size, block_allocation_size>>>(
      height, width, stride, matrix_gpu, averages_gpu);
  TIME_END();

  /* Copy the results back */

  TIME_START();
  cudaMemcpy(averages, averages_gpu, height * sizeof(FP_TYPE),
