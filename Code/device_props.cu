...
int max_grid_dim_x(const int device_index) {
  ...
  if (cudaDeviceGetAttribute(&dim, cudaDevAttrMaxGridDimX, device_index) !=
      cudaSuccess) {
    return -1;
  }
 ...
 * @return int The maximum x dimension. Negative on error.
 */
int current_max_grid_dim_x(void) { return max_grid_dim_x(current_device()); }
 ...
 * @return int The number of CUDA cores. Negative on error.
 */
int current_cuda_core_count(void) { return cuda_core_count(current_device()); }
