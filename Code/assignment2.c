 ...
 * @brief A program to demonstrate the iteration and averaging function.
 * @version 1.0
 * @date 2022-04-04
 * 
 * 
...
int main(int argc, char *argv[])
{
    ...
    const int iteration_block_size = (block_size + 2 > MAX_BLOCK_SIZE)
                                         ? MAX_BLOCK_SIZE
    ...
    apply_iterations_gpu(iterations, height, width,
                         matrix_cuda, iteration_block_size, iteration_timings);
    const double iteration_total_time = get_duration(&start_time); /* Stores the time as measured by the CPU */

    /* Calculate differences in results */

    const double matrix_diff = L1_diff(height, width,
                                       stride, matrix,
                                       width, matrix_cuda); /* The average difference */

    const int matrix_disc = count_discrepancies(tolerance,
                                                height, width,
                                                stride, matrix,
                                                width, matrix_cuda); /* The count of large errors */
    ...
    printf("Device:\t\t%d\n", device_index);
    printf("Block size:\t%d (+%d)\n\n", block_size, iteration_block_size - block_size);
    ...
    if (show_timing)
    {
        ...
        printf("\tSetup\t\t\t\t%4.2e (%05.2lf%%)\n", iteration_timings[0], 100 * iteration_timings[0] / iteration_total_time);
        printf("\tAllocation\t\t\t%4.2e (%05.2lf%%)\n", iteration_timings[1], 100 * iteration_timings[1] / iteration_total_time);
        printf("\tTransfer to\t\t\t%4.2e (%05.2lf%%)\n", iteration_timings[2], 100 * iteration_timings[2] / iteration_total_time);
        printf("\tComputation\t\t\t%4.2e (%05.2lf%%)\n", iteration_timings[3], 100 * iteration_timings[3] / iteration_total_time);
        printf("\tTransfer from\t\t\t%4.2e (%05.2lf%%)\n", iteration_timings[4], 100 * iteration_timings[4] / iteration_total_time);
    }
    ...
    if (do_average)
    {
        ...
        get_duration(&start_time);
        average_rows_gpu(height, width,
                         width, matrix_cuda,
                         averages_cuda, block_size, average_timings);
        const double average_total_time = get_duration(&start_time); /* Stores the time as measured by the CPU */

        /* Get the overall average on the GPU */

        FP_TYPE overall_average_cuda = 0.0f; /* Will store the average across all rows */
        average_rows_gpu(1, height,
                         height, averages_cuda,
                         &overall_average_cuda,
                         block_size, NULL);

        /* Calculate differences in results */

        const double average_diff = L1_diff(height, 1,
                                            1, averages,
                                            1, averages_cuda); /* The average difference */

        const int average_disc = count_discrepancies(tolerance,
                                                     height, 1,
                                                     1, averages,
                                                     1, averages_cuda); /* The count of large errors */
        ...
        if (show_timing)
        {
            ...
            printf("\tTIMINGS\t\tCPU\t\tGPU\n\n");
            printf("\tTotal time\t%4.2e\t%4.2e\n\n", cpu_timings[1], average_total_time);
            printf("\tSetup\t\t\t\t%4.2e (%05.2lf%%)\n", average_timings[0], 100 * average_timings[0] / average_total_time);
            printf("\tAllocation\t\t\t%4.2e (%05.2lf%%)\n", average_timings[1], 100 * average_timings[1] / average_total_time);
            printf("\tTransfer to\t\t\t%4.2e (%05.2lf%%)\n", average_timings[2], 100 * average_timings[2] / average_total_time);
            printf("\tComputation\t\t\t%4.2e (%05.2lf%%)\n", average_timings[3], 100 * average_timings[3] / average_total_time);
            printf("\tTransfer from\t\t\t%4.2e (%05.2lf%%)\n", average_timings[4], 100 * average_timings[4] / average_total_time);
        }
        ...
        if (show_csv)
        {
            fprintf(stderr, "%d,%d,%d,%lu,", height, width, iterations, 8 * sizeof(FP_TYPE));
