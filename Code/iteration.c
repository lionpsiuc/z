 ...
 */
static void iterate_row(const int width, FP_TYPE* const restrict row,
...
#ifdef USE_DOUBLE
    row[j] = 0.34 * row_swap[j - 2] + 0.28 * row_swap[j - 1] +
             0.2 * row_swap[j] + 0.12 * row_swap[j + 1] +
             0.06 * row_swap[j + 2];
#else
    row[j] = 0.34f * row_swap[j - 2] + 0.28f * row_swap[j - 1] +
             0.2f * row_swap[j] + 0.12f * row_swap[j + 1] +
             0.06f * row_swap[j + 2];
#endif
  }
...
static void apply_iterations_row(const int iterations, const int width,
                                 FP_TYPE* restrict row,
                                 FP_TYPE* restrict row_swap) {
  if (iterations % 2 != 0) {
 ...
 * @param height The height of the matrix.
 * @param width The width (excluding the extra colums) of the matrix.
 * @param matrix The matrix to store the result. Should be have height X (width
 * + 2) entries allocated.
 * @param matrix_swap A matrix with the same dimensions used to store
 * intermediary values.
 */
void apply_iterations(const int iterations, const int height, const int width,
  ...
  for (int i = 0; i < height; i++) {
    FP_TYPE* const row =
        matrix + i * stride; /* A pointer to the row in matrix */
    FP_TYPE* const row_swap =
        matrix_swap + i * stride; /* A pointer to the row in matrix_swap */
