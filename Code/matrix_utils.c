 ...
 */
void print_matrix(const int height, const int width, const int stride,
                  FP_TYPE* const A) {
 ...
 */
int count_discrepancies(const double tolerance, int height, const int width,
  ...
  for (int i = 0; i < height; i++) {
    for (int j = 0; j < width; j++) {
      ...
      if (fabs((double) A[stride_A * i + j] - B[stride_B * i + j]) >=
          tolerance) {
        count++;
      }
    }
 ...
 */
double L1_diff(const int height, const int width, const int stride_A,
               const FP_TYPE* const A, const int stride_B,
               const FP_TYPE* const B) {
  // I used doubles since this is a diagnostic function
  // and not performance sensitive.
  double diff = 0.0;
  for (int i = 0; i < height; i++) {
