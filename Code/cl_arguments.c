 ...
 */
static void read_argument(const char argument_flag,
                          const void *const variable,
                          const char format[])
{
    if (sscanf(optarg, format, variable) != 1)
    {
        fprintf(stderr,
                "Couldn't read argument for -%c. Exiting.\n",
                argument_flag);
        print_help();
        exit(EXIT_FAILURE);
    }
...
CL_arguments parse_arguments(const int argc, char *const argv[])
{
    ...
    while ((argument_flag = getopt(argc, argv, argument_list)) != -1)
    {
        ...
        {
        case 'n':
            read_argument(argument_flag, &(arguments.height), "%d");
            break;
        case 'm':
            read_argument(argument_flag, &(arguments.width), "%d");
            break;
        case 'p':
        ...
        case 'b':
            ...
            break;
        case 'a':
            arguments.do_average = true;
            break;
        case 'c':
            arguments.skip_cpu = true;
            break;
        case 't':
            arguments.show_timing = true;
            break;
        case 'v':
            arguments.show_values = true;
            break;
        case 'o':
            arguments.show_csv = true;
            break;
        case 'h':
            print_help();
            exit(EXIT_SUCCESS);
