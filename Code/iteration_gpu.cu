 ...
 */
inline __device__ FP_TYPE iterate_point(const FP_TYPE old_m2, const FP_TYPE old_m1,
                                        const FP_TYPE old,
                                        const FP_TYPE old_p1, const FP_TYPE old_p2)
{
    //Using float constants in the single precision case massively improves performance.
#ifdef USE_DOUBLE
    return 0.34 * old_m2 + 0.28 * old_m1 + 0.2 * old + 0.12 * old_p1 + 0.06 * old_p2;
#else
    // return 0.34 * old_m2 + 0.28 * old_m1 + 0.2 * old + 0.12 * old_p1 + 0.06 * old_p2;
    return 0.34f * old_m2 + 0.28f * old_m1 + 0.2f * old + 0.12f * old_p1 + 0.06f * old_p2;
#endif
}

/**
 * @brief Applies the iteration once to the entries on the interior of the block's
 * allocation (that is, all but the first and last two). 
 *
 * This is the function executed by all but two of the threads in the block.
 * 
 * @param interior_row The vector into which the newly calculated interior values
 * should be stored.
 * @param interior_row_swap A vector containing the previous iteration's interior
 * values.
 * @param thread_steps The number of entries the calling thread should calculate.
 * @param thread_stride The gap between subsequent entries calculated by the
 * thread.
 */
__device__ void iterate_interior(FP_TYPE *const __restrict__ interior_row, const FP_TYPE *const __restrict__ interior_row_swap,
                                 const int thread_steps, const int thread_stride)
{
    ...
    for (int i = 0; i < thread_steps; i++)
    {
        ...
        interior_row[index] = iterate_point(interior_row_swap[index - 2], interior_row_swap[index - 1],
                                            interior_row_swap[index],
                                            interior_row_swap[index + 1], interior_row_swap[index + 2]);
    }
 ...
 */
__device__ void iterate_boundary(FP_TYPE *const __restrict__ thread_values, const FP_TYPE *const __restrict__ thread_values_swap,
                                 volatile Boundary *const __restrict__ out_bound, volatile Boundary *const __restrict__ in_bound,
                                 const Edge edge, const FP_TYPE edge_values[2])
{
    ...
    const Side side = (threadIdx.x % 2 == 0) ? LEFT_SIDE : RIGHT_SIDE; /* The side of the block's allocation we're on */
    FP_TYPE in_values[2];                                              /* Holds the values from outside the block's allocation*/

    /* Communicate and read in in_values */

    if (edge == NOT_EDGE) /* If we're not on an edge we need to communicate with another block */
    {
        /* Update out_bound */
    ...
    {
    case LEFT_SIDE:
        thread_values[0] = iterate_point(in_values[0], in_values[1],
                                         thread_values_swap[0],
                                         thread_values_swap[1], thread_values_swap[2]);

        thread_values[1] = iterate_point(in_values[1], thread_values_swap[0],
                                         thread_values_swap[1],
    ...
    case RIGHT_SIDE:
        ...
        thread_values[0] = iterate_point(thread_values_swap[-2], thread_values_swap[-1],
                                         thread_values_swap[0],
                                         thread_values_swap[1], in_values[0]);

        thread_values[1] = iterate_point(thread_values_swap[-1], thread_values_swap[0],
                                         thread_values_swap[1],
...
__device__ void iterate_row_block(const int local_width,
                                  FP_TYPE *const block_row, FP_TYPE *const block_row_swap,
                                  volatile Boundary *const out_bounds[2], volatile Boundary *const in_bounds[2],
                                  const Edge edges[2], const FP_TYPE edge_values[2])
{
    const int interior_threads = blockDim.x - 2; /* The number of threads doing the standard iteration */

    if (threadIdx.x < interior_threads) /* If we're in the interior we call iterate_interior() */
    {
        const int interior_points = local_width - 4; /* The number of non-boundary entries */

        FP_TYPE *const interior_row = block_row + 2;           /* Pointer to the first interior entry in the output */
        FP_TYPE *const interior_row_swap = block_row_swap + 2; /* Pointer to the first interior entry in the old values */

        const int thread_steps = div_up(interior_points - threadIdx.x, interior_threads); /* The number of entries for the thread to iterate */

        iterate_interior(interior_row, interior_row_swap, thread_steps, interior_threads);
    }

    else /* If the thread is handling a boundary we call iterate_boundary() */
    {
        const Side side = (threadIdx.x % 2 == 0) ? LEFT_SIDE : RIGHT_SIDE; /* The side of the block's allocation the thread handles */
        const Edge edge = edges[side];                                     /* Describes whether the thread handles an edge */

        const int thread_offset = (side == LEFT_SIDE) ? 0 : local_width - 2; /* Offset for the entries the thread handles */
        FP_TYPE *const thread_values = block_row + thread_offset;            /* Pointer to the first entry for the thread's output */
        FP_TYPE *const thread_values_swap = block_row_swap + thread_offset;  /* Pointer to the first entry for the thread's old values */

        iterate_boundary(thread_values, thread_values_swap, out_bounds[side], in_bounds[side], edge, edge_values);
    }
 ...
 */
inline __device__ FP_TYPE initial_value(const int height, const int row, const int col)
{
    switch (col)
    {
...
#ifdef USE_DOUBLE
    case 0:
        return (FP_TYPE)(row + 1) / (FP_TYPE)(height);
    case 1:
        return 0.8 * (FP_TYPE)(row + 1) / (FP_TYPE)(height);
    default:
        return 1.0 / 15360.0;
#else
    case 0:
        return (FP_TYPE)(row + 1) / (FP_TYPE)(height);
    case 1:
        return 0.8f * (FP_TYPE)(row + 1) / (FP_TYPE)(height);
    default:
        return 1.0f / 15360.0f;
#endif
    }
}
...
__device__ void set_initial_conditions_block(const int height, const int row,
                                             const int block_offset, const int block_width,
...
/**
 * @brief A kernel to apply the given number of iterations to the initial values.
 * 
 ...
 * communication.
 */
__global__ void apply_iterations_kernel(const int iterations,
                                        ...
                                        volatile Boundary bounds[])
{
    ...
    const int entries_per_block = div_up(width, blocks_per_row); /* The most entries allocated to a given block */
    const int rows_per_grid = gridDim.x / blocks_per_row;        /* The number of rows being worked on at once */
    ...
    const int block_width = allocation(width, blocks_per_row, block_row_index); /* The number of entries allocated to the block */
    const int block_offset = offset(width, blocks_per_row, block_row_index);    /* The column of the first entry allocated to the block */

    const int block_steps = div_up(height - start_row, rows_per_grid); /* The number of rows to iterate */

    const Edge edges[2] = {(block_row_index == 0)
                               ? LEFT_EDGE
                               : NOT_EDGE,
                           (block_row_index == blocks_per_row - 1)
                               ? RIGHT_EDGE
                               : NOT_EDGE}; /* Describes whether the block's allocation lies on an edge */

    /* Set up pointers to shared memory */

    extern __shared__ FP_TYPE block_shared[]; /* A pointer to the start of the dynamically allocated shared memory */

    FP_TYPE *const block_row = block_shared + 0;                   /* The vector to store the finished values */
    FP_TYPE *const block_row_swap = block_row + entries_per_block; /* A vector to store itermediary values */

    /* Set up pointers to global memory */

    volatile Boundary *in_bounds[2] = {(edges[LEFT_SIDE] == LEFT_EDGE)
                                           ? NULL
                                           : bounds + 2 * blockIdx.x - 1,
                                       (edges[RIGHT_SIDE] == RIGHT_EDGE)
                                           ? NULL
                                           : bounds + 2 * blockIdx.x + 2}; /* Used for incoming communication on the bounds */

    volatile Boundary *out_bounds[2] = {(edges[LEFT_SIDE] == LEFT_EDGE)
                                            ? NULL
                                            : bounds + 2 * blockIdx.x,
                                        (edges[RIGHT_SIDE] == RIGHT_EDGE)
                                            ? NULL
                                            : bounds + 2 * blockIdx.x + 1}; /* Used for outgoing communication on the bounds */
    ...
    for (int i = 0; i < block_steps; i++)
    {
        ...
        const FP_TYPE edge_values[2] = {initial_value(height, row, 0),
                                        initial_value(height, row, 1)}; /* The first values on the current row */
        ...
        __syncthreads();
        set_initial_conditions_block(height, row, block_offset, block_width, block_row);
        set_initial_conditions_block(height, row, block_offset, block_width, block_row_swap);
        ...
        {
            __syncthreads();
            iterate_row_block(block_width, block_row, block_row_swap, out_bounds, in_bounds, edges, edge_values);
        }
        for (int iter = 0; iter < (iterations / 2); iter++)
        {
            __syncthreads();
            iterate_row_block(block_width, block_row_swap, block_row, out_bounds, in_bounds, edges, edge_values);
        ...
        FP_TYPE *const offset_matrix = matrix_gpu + row * width + block_offset; /* A pointer to the block's share of the row in global memory */
 ...
 */
int apply_iterations_gpu(const int iterations,
                         const int height, const int width,
                         ...
                         const int block_size, float timings[5])
{
    ...
    TIME_START();
    const int max_shared_size = current_max_shared_per_block();                /* The most shared memory per block supported by the card */
    const int max_entries_per_block = max_shared_size / (2 * sizeof(FP_TYPE)); /* The largest row that can fit in a block's shared memory */
    // const int max_entries_per_block = 5; /* Useful for debugging */

    //This would be the x dimension if we used a 2D grid.
    const int blocks_per_row = div_up(width, max_entries_per_block);           /* The number of blocks that will work on a given row */
    const int entries_per_block = div_up(width, blocks_per_row);               /* The most entries allocated to a given block */
    const int block_allocation_size = entries_per_block * 2 * sizeof(FP_TYPE); /* The size of each block's dynamically allocated shared memory */

    //For best performance we can give the GPU as many blocks to work with as possible.
    const int rows_per_grid = height; /* An upper bound on the number of rows for the grid to work on at once */
    ...
    // const int sm_count = current_multiprocessor_count();               /* The number of SMs on the selected card */
    // const int max_entries_per_grid = max_entries_per_block * sm_count; /* The number of entries that can fit in the card's shared memory */
    ...
    // {
    //     fprintf(stderr, "Too many columns to fit in shared memory. Exiting.\n");
    //     exit(EXIT_FAILURE);
    // }
    ...
    // const int entries_per_row = entries_per_block * blocks_per_row;                /* The number of entries stored in shared memory for a row (might be more than width) */
    // const int rows_per_grid = MIN(height, max_entries_per_grid / entries_per_row); /* An upper bound on the number of rows for the grid to work on at once */


    //We choose the grid size to be a multiple of blocks_per_row
    const int max_grid_size = (current_max_grid_dim_x() / blocks_per_row) * blocks_per_row; /* The maximum usable grid size */
    const int grid_size = MIN(max_grid_size, blocks_per_row * rows_per_grid);               /* The grid size to use */
    ...
    // printf("Row decomp:\t%d = %d x %d\n", width, blocks_per_row, entries_per_block);
    ...
    {
        cudaFuncSetAttribute(apply_iterations_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, max_shared_size);
    }
    TIME_END();
    ...
    if (cudaMalloc((void **)(&matrix_gpu), height * width * sizeof(FP_TYPE)) != cudaSuccess)
    {
    ...
    if (cudaMalloc((void **)(&bounds_gpu), 2 * grid_size * sizeof(Boundary)) != cudaSuccess)
    {
        fprintf(stderr, "Failed to allocate memory on device. Exiting.\n");
        exit(EXIT_FAILURE);
    }
    ...
    TIME_START();
    apply_iterations_kernel<<<grid_size, block_size, block_allocation_size>>>(iterations, height, width, matrix_gpu, blocks_per_row, bounds_gpu);
    TIME_END();

    /* Copy results to host */

    TIME_START();
    cudaMemcpy(matrix, matrix_gpu, height * width * sizeof(FP_TYPE), cudaMemcpyDeviceToHost);
    TIME_END();
