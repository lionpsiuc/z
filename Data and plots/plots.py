...
data = data.groupby(
    ['block_size', 'height', 'width', 'device_index'], as_index=False).median()
data = data.drop('cpu_iteration', axis=1)
data = data.drop('cpu_average', axis=1)

data = data.merge(cpu_data, left_on=['height', 'width'], right_on=[
    'height', 'width'], suffixes=(False, False))
data = data[['block_size', 'height', 'width', 'device_index',
...
for speedup in ['iter_speedup_total', 'iter_speedup_comp',
                'avg_speedup_total', 'avg_speedup_comp']:
