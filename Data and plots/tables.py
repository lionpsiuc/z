...
data = data.groupby(
    ["block_size", "height", "width", "precision", "device_index"], as_index=False
).median()
...
data = data.merge(
    cpu_data,
    left_on=["height", "width", "precision"],
    right_on=["height", "width", "precision"],
...
iter_data = data.loc[
    ((data.device_index == 0) & (data.block_size == 256))
    | ((data.device_index == 1) & (data.block_size == 992))
]
...
avg_data = data.loc[
    ((data.device_index == 0) & (data.block_size == 32))
    | ((data.device_index == 1) & (data.block_size == 32))
]
